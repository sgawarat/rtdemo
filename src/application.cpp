...
void register_managed_technique(const uint64_t* layout_ids, size_t layout_id_count, const char* name, tech::Technique* technique) {
...
void register_managed_scene(const uint64_t* layout_ids, size_t layout_id_count, const char* name, scene::Scene* scene) {
    for (size_t i = 0; i < layout_id_count; ++i) {
...
void Application::update() {
    if (ImGui::Combo("current layout", &current_layout_index, layout_names.data(), layout_names.size())) {
    ...
    if (current_technique_list) {
        if (ImGui::Combo("current technique", &current_technique_index, current_technique_list->names().data(), current_technique_list->names().size())) {
            current_technique = current_technique_list->techniques()[current_technique_index];
            current_technique->restore();
        }
    }

    if (current_scene_list) {
        if (ImGui::Combo("current scene", &current_scene_index, current_scene_list->names().data(), current_scene_list->names().size())) {
